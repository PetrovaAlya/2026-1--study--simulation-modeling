...
# Пути к файлам (ручные, без DrWatson)
project_root = dirname(@__DIR__)
srcdir() = joinpath(project_root, "src")
datadir() = joinpath(project_root, "data", "exp_pro")
...
include(joinpath(srcdir(), "ross_logic.jl"))
...
function run_experiment(current_N)
    ...
    try
        ...
        msg = string(e)
        if !occursin("StopSimulation", msg)
            rethrow(e)
        end
    end
...
for test_N in [5, 10, 15]
    ...
    rho = MU / (LAMBDA * test_N)  # Теоретический расчёт
...
# Сохранение
CSV.write(joinpath(datadir(), "ross_final_metrics.csv"), summary_df)
savefig(p_spares, joinpath(plotsdir(), "ross_dynamics_plot.png"))